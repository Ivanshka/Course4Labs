...
excludingDataFrame = newDataFrame.drop(index=dataFrame.index[dataFrame.isna().sum(axis=1) == dataFrame.isna().sum(axis=1).max()])
print("excluding values")
print(excludingDataFrame)
#3.filleng NAN values
newDataFrameAVG=excludingDataFrame.fillna(excludingDataFrame.mean())
print("AVG")
print(newDataFrameAVG)
newDataFrameModa=excludingDataFrame.fillna(excludingDataFrame['Girth'].mode()[0])
print("MODA")
print(newDataFrameModa)
newDataFrameFFill=excludingDataFrame.fillna(method="ffill")
